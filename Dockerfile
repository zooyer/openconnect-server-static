...
CFLAGS="-Wno-analyzer-fd-leak -Wno-analyzer-null-dereference -Wno-analyzer-use-of-uninitialized-value -Wno-type-limits -Wno-unused-macros -Wno-stringop-overflow -march=generic-armv7-a -mtune=cortex-a5 -mfpu=vfp" \
./configure \
	--prefix=/usr \
	--enable-static=yes \
	--enable-shared=no \
...
CFLAGS="-Wno-type-limits -march=generic-armv7-a -mtune=cortex-a5 -mfpu=vfp" \
LDFLAGS="-L/usr/local/lib -s -w -static -march=generic-armv7-a -mtune=cortex-a5 -mfpu=vfp" \
./configure \
	--with-local-talloc \
	--with-pager="" \
	--without-geoip \
