...
CFLAGS="-Wno-analyzer-fd-leak -Wno-analyzer-null-dereference -Wno-analyzer-use-of-uninitialized-value -Wno-type-limits -Wno-unused-macros -Wno-stringop-overflow -march=armv7-a -mtune=cortex-a5 -mfpu=vfp" \
./configure \
	--prefix=/usr \
	--enable-static=yes \
	--enable-shared=no \
...
CFLAGS="-Wno-type-limits -march=armv7-a -mtune=cortex-a5 -mfpu=vfp" \
LDFLAGS="-L/usr/local/lib -s -w -static -march=armv7-a -mtune=cortex-a5 -mfpu=vfp" \
./configure \
	--with-local-talloc \
	--with-pager="" \
	--without-geoip \
